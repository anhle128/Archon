...
```sh
bun run build:web
cd e2e && env -i PATH="$PATH" HOME="$HOME" npm run typecheck
cd e2e && npx playwright test -c playwright.config.ts workflow-run-hitl-room.spec.ts
cd e2e && npx playwright test -c playwright.config.ts workflow-run-hitl-visual.spec.ts
cd e2e && npm run test:ui:hitl
bun run validate
```

Captures: `plans/260907-1454-workflow-run-hitl-mockup-alignment/reports/captures/` (`{console,legacy}-{actual,mockup}-{1440x1000,1024x900,768x900,390x844}.png`).
...
| 390×844   | Legacy  | single-pane                           | n/a                                           | Room remains open in captures                                               | None observed       | Back                                               | matched |

### 1440×1000 rail and zoom
...
- Browser zoom 200% was **not** driven as an OS/browser chrome zoom in this headless pass. Playwright `setViewportSize` does not substitute for that. Treat 200% zoom as a remaining manual check.
...
- **200% browser zoom:** not established in this headless run (see rail/zoom).
- **Full `npm run test:ui` (all UI specs):** the Story 5.6 gate is the HITL/room/visual suites plus `bun run validate`. Unrelated usage specs are outside this story's acceptance command list in plan Step 6.

## Deviations

None on the automated viewports above. The 200% zoom observation is a named gap, not a product mismatch.
