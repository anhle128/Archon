...
- [x] [Review][Patch] R3-F1 — The R2-F2 fix leaves story and review-finding identifiers in a workflow test artifact, violating the project rule that code and test artifacts must not contain plan, story, epic, or finding identifiers [packages/workflows/src/defaults/v2-tr-join-contract.test.ts:306]. FIXED: replaced all story/finding identifiers (R1-F3, a4.1, a3.2, a3.3, R1-F6) in four test files with stable domain wording; extended TD-041 and TD-164 guard regexes to catch R\\d-F\\d and lowercase a\\d.\\d patterns.
...
- R3-F1 fix pass: removed all story/finding identifiers (R1-F3, a4.1, a3.2, a3.3, R1-F6) from four test files; replaced with stable domain wording; extended TD-041 regex (TR-join) to catch `R\d-F\d` and `a\d.\d`; extended TD-164 regex (quality-summary) to catch `a[0-9]\.[0-9]`; all tests pass; `bun run validate` passes.
...
- `packages/workflows/src/defaults/v2-quality-summary-contract.test.ts` — UPDATED: aligned assertions with story spec (8 deps, quality-gate-summary output_type); added TD-170 CR round validation tests; added TD-180 producer-source tracking tests (4); R3-F1 fix removed story identifiers from header and TD-163 assertion, extended TD-164 guard regex.
- `packages/workflows/src/defaults/v2-quality-summary-dag.test.ts` — UPDATED: added TD-170 DAG tests for negative, fractional, and zero CR round values (all fail closed); added TD-180 DAG tests for real output self-identifying as skipped sibling (3); R3-F1 fix removed story identifier from TD-111 comment.
- `packages/workflows/src/defaults/v2-tr-join-contract.test.ts` — UPDATED: TD-042 dep set expanded to 8, output_type changed to quality-gate-summary; R2-F2 fix updated TD-042 description and assertions to reflect a4.1 FAIL semantics; R3-F1 fix removed story/finding identifiers from TD-038 and TD-042 headers, extended TD-041 guard regex.
- `packages/workflows/src/defaults/v2-tr-join-dag.test.ts` — UPDATED: TD-043 updated to match a4.1 evolved behavior (FAIL is a valid routing decision, not a hard error); R3-F1 fix removed story/finding identifiers from TD-043, TD-044, TD-045, TD-046, TD-047 headers.
