...
- **v1 release gate (owner-ratified 2026-09-15).** Steering v1 ships at the universal floor — interrupt + `Queue`, every message `sent`, on every provider. `delivered` (G1 · claude SDK ≥ 0.3.246), claude soft-inject (G2 · spike), and grok hooks (G3 · spike) are **post-v1 gated backlog**, each on its own external gate; none blocks the v1 release.
...
> **Post-v1 mapping (2026-09-15):** the soft-inject and `delivered` items below are the post-v1 gated backlog outside the v1 release gate — claude soft-inject = **G2**, grok hooks = **G3**, the `delivered` chip = **G1**. The v1 floor (interrupt + `Queue` + `sent`) does not depend on any of them.
