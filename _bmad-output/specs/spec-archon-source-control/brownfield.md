...
- `working_path` is written to the run's cwd by `executeWorkflow` (`executor.ts:1398-1404`); folder projects run in-place at `workingCwd` (`workflow.ts:1970-1977`) and `--no-worktree` runs at `cwd` (`workflow.ts:2088-2089`) — so it is **not** null for those. A null `working_path` is a missing/legacy state → CAP-6. After the container-backend gate (architecture spine AD-6), every remaining run is read the same way by `working_path`; host availability is directory-exists AND is-a-git-checkout at read time. A non-git folder fails that check → CAP-6 `no_checkout`. Do not branch on codebase `kind`. A missing `conversation.isolation_env_id` is not CAP-6.
...
- `log` — commit history for the worktree branch (records include `parents[]`).
- `show` / `diff` — `M`: hunks (`HEAD → worktree` Now; `parent → commit` history); `A`/`D`: file content. Commit-scoped reads: `ls-tree -z` + `cat-file blob`, never `<oid>:<path>`. Transport, containment, and route shape: architecture spine AD-1–AD-4.
