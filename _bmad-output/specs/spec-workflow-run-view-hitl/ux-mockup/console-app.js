/* Archon run-view HITL mockup — Command Center surface (console.html).
 *
 * Same design language as the Legacy mockup (index.html), mapped onto the
 * console's log-first idiom:
 *  - Log view   = the run stream: one section per node RUN (never merged),
 *                 Ask / gate cards render INLINE at the awaiting node
 *  - Graph view = the same layered-DAG renderer as the Legacy mockup
 *  - NodePanel  = the same docked right panel (iteration chips, per-run log)
 *  - Composer   = the console's "Reply…" box
 *
 * The simulation walks the same speckit-ralph-native-feature shape as the
 * Legacy mockup so both surfaces can be compared 1:1.
 */
'use strict';
...
// ---------- layered DAG layout (identical to the Legacy mockup) ----------
const NODE_W = 208;
const NODE_H = 58;
const GAP_X = 34;
const GAP_Y = 46;
...
function computeLayout() {
  ...
  const reorder = () => {
    layers.forEach(l => l.forEach((id, k) => posInLayer.set(id, k)));
  };
  reorder();
...
// ---------- graph rendering + viewport (identical to the Legacy mockup) ----------
let LAYOUT = null;
...
// Curved bezier routing with distance-aware ports (same rules as Legacy):
//  - short edge (adjacent layers / small offset) → curve into the TOP
//  - long edge with a clear side offset → S-curve into the SIDE facing the source
function edgePath(e) {
...
function startRun(nodeId, opts = {}) {
  ...
  ns.status = 'running';
  renderStreamSection(r);
  paintGraphNode(nodeId);
  qs('#stat-runs').textContent = String(state.runs.length);
  return r;
}
...
function addItem(r, item) {
  r.items.push(item);
  const body = qs('#ccnode-' + r.hid + ' .cc-body');
  if (body) {
    body.appendChild(renderItem(r, item));
    streamScroll();
  }
  if (state.panelOpen && state.selected === r.nodeId && state.panelRun[r.nodeId] === r.hid) {
    const pbody = qs('#panel-body');
    if (pbody) {
      const empty = qs('.panel-empty', pbody);
      if (empty) empty.remove();
      pbody.appendChild(renderItem(r, item));
      pbody.scrollTop = pbody.scrollHeight;
    }
  }
}

// ---------- log stream (one section per RUN, cards inline at the node) ----------
function streamScroll() {
  const v = qs('#cc-view-log');
  if (v.scrollHeight - v.scrollTop - v.clientHeight < 220) v.scrollTop = v.scrollHeight;
}

function renderStreamSection(r) {
  const stream = qs('#cc-stream');
  const d = nodeDef(r.nodeId);
  const sec = el('section', 'cc-node status-' + r.status);
  sec.id = 'ccnode-' + r.hid;
  sec.dataset.node = r.nodeId;

  const div = el('button', 'cc-divider');
  div.type = 'button';
  div.title = 'Open this run in the node panel';
  div.appendChild(el('span', 'cc-dot', '●'));
  const name = el('span', 'cc-name mono', r.nodeId);
  if (r.iter) name.appendChild(el('span', 'lr-iter', '×' + r.iter));
  if (r.seq > 1 && !r.iter) name.appendChild(el('span', 'lr-iter lr-rerun', '#' + r.seq));
  div.appendChild(name);
  div.appendChild(el('span', 'type-pill type-' + d.kind, kindLabel(d)));
  div.appendChild(el('span', 'cc-status', STATUS_TEXT[r.status]));
  div.appendChild(el('span', 'cc-dur mono', ''));
  div.appendChild(el('span', 'cc-start mono', fmtClock(Math.floor((r.startMs - state.t0) / 1000))));
  div.addEventListener('click', () => openPanel(r.nodeId, r.hid));

  sec.appendChild(div);
  sec.appendChild(el('div', 'cc-body'));
  stream.appendChild(sec);
  streamScroll();
}

function paintStreamSection(r) {
  const sec = qs('#ccnode-' + r.hid);
  if (!sec) return;
  sec.classList.remove('status-pending', 'status-running', 'status-awaiting', 'status-completed', 'status-failed', 'status-skipped');
  sec.classList.add('status-' + r.status);
  ...
  qs('.cc-status', sec).textContent = text;
  qs('.cc-dur', sec).textContent = r.durMs ? fmtDur(r.durMs) : '';
}

// a run-level note rendered into the node's stream section (not stored as an item)
function streamNote(r, text) {
  const body = qs('#ccnode-' + r.hid + ' .cc-body');
  if (!body) return;
  const n = el('div', 'psys cc-note');
  n.innerHTML = md(text);
  body.appendChild(n);
  streamScroll();
}

// a run-level system line between sections
function streamSys(text) {
  qs('#cc-stream').appendChild(el('div', 'cc-sysline', text));
  streamScroll();
}

// Ask / gate cards render INLINE, inside the awaiting node's stream section
function streamCard(nodeId) {
  const r = latestRun(nodeId);
  const body = qs('#ccnode-' + r.hid + ' .cc-body');
  body.appendChild(askCard(nodeId));
  streamScroll();
}
function streamGate(nodeId) {
  const r = latestRun(nodeId);
  const body = qs('#ccnode-' + r.hid + ' .cc-body');
  body.appendChild(gateCard(nodeId));
  streamScroll();
}

// ---------- shared node panel (same component as the Legacy mockup) ----------
...
// ---------- item renderers (shared by the stream and the panel) ----------
...
function jumpToCard(id) {
  switchView('log');
  const card = qs('#ask-' + id) || qs('#gate-' + id);
...
function askSummary(id) {
  ...
  const go = el('button', 'btn btn-primary ask-goto', 'Jump to the card in the stream →');
  go.type = 'button';
  go.addEventListener('click', () => jumpToCard(id));
  w.appendChild(go);
  return w;
}
...
// ---------- ask card (interactive, inline in the stream) ----------
function askCard(nodeId) {
  ...
  submit.addEventListener('click', () => {
    ...
    if (r) {
      ...
      streamNote(r, 'completed — answer delivered');
    }
    syncAwaiting();
...
// ---------- gate card (plannotator_gate, inline in the stream) ----------
function gateCard(nodeId) {
  ...
  approve.addEventListener('click', () => {
    ...
    setRunStatus(r, 'completed');
    streamNote(r, 'approved' + (comment.value ? ' — “' + comment.value + '”' : ''));
  ...
  annotate.addEventListener('click', () => {
    ...
    setRunStatus(r, 'completed');
    streamNote(r, 'annotations sent — rework applied');
    card.replaceWith(gateCard(nodeId));
    continueAfterGate(nodeId);
  });
...
// ---------- awaiting state → the header status pill ----------
function syncAwaiting() {
  const badge = qs('#run-status');
  if (badge.classList.contains('status-completed') || badge.classList.contains('status-failed')) return;
  const awaiting = state.runs.filter(r => r.status === 'awaiting');
  badge.classList.toggle('status-awaiting', awaiting.length > 0);
  badge.classList.toggle('status-running', awaiting.length === 0);
  qs('.sb-label', badge).textContent =
    awaiting.length > 0 ? 'Awaiting input (' + awaiting.length + ')' : 'Running';
  badge.style.cursor = awaiting.length ? 'pointer' : '';
  badge.onclick = awaiting.length ? () => jumpToCard(awaiting[0].nodeId) : null;
}

// ---------- simulation (same beats as the Legacy mockup) ----------
function quick(r, items, doneNote) {
  items.forEach((item, k) => at(250 + k * 320, () => addItem(r, item)));
  at(250 + items.length * 320 + 150, () => {
    setRunStatus(r, 'completed');
    if (doneNote) streamNote(r, doneNote);
  });
}

function simulate() {
  streamSys('Run started · workflow speckit-ralph-native-feature · starter dale');
  streamSys('Trigger: “speckit ralph native — github.com/oceanlabs-holding/x10.gigo.harness-service/issues/123”');
  ...
  at(2900, () => {
    ...
    at(800, () => {
      ...
      streamNote(r, 'is awaiting input — AskHuman — 2 questions');
      streamCard('clarify');
    });
  });
}
...
function continueAfterClarify() {
  ...
  at(3000, () => {
    ...
    at(600, () => {
      setRunStatus(r, 'awaiting');
      streamNote(r, 'is awaiting decision — live Plannotator review');
      streamGate('clarify-gate');
    });
  });
}
...
function continueAfterGate(gateId) {
  ...
  if (gateId === 'speckit-converge-review-gate') {
    streamSys('Retry loop — re-running Ralph with the reviewed tasks');
...
function runRalphIterations(pass, count) {
  ...
  const next = () => {
    ...
    streamNote(r, 'started — ' + stories[i]);
    ...
    at(1700, () => {
      setRunStatus(r, 'completed');
      streamNote(r, 'completed — ' + stories[i]);
      i += 1;
    ...
    if (route === 'negative') {
      at(500, () => {
        ...
        at(600, () => {
          ...
          streamNote(g, 'is awaiting decision — review added tasks');
          streamGate('speckit-converge-review-gate');
        });
      });
    }
    if (route === 'positive') {
...
function finishRun(status) {
  const badge = qs('#run-status');
  badge.classList.remove('status-running', 'status-awaiting');
...
// ---------- chrome: view switch, filter, toggles, composer, replay, split ----------
function switchView(name) {
  qsa('#cc-viewbtns .cc-vb').forEach(b => b.classList.toggle('active', b.dataset.view === name));
  qs('#cc-view-log').classList.toggle('hidden', name !== 'log');
  qs('#cc-graph').classList.toggle('hidden', name !== 'graph');
  qs('#cc-view-artifacts').classList.toggle('hidden', name !== 'artifacts');
  if (name === 'graph' && LAYOUT) fitGraph();
}

function bindChrome() {
  qsa('#cc-viewbtns .cc-vb').forEach(b => b.addEventListener('click', () => switchView(b.dataset.view)));

  qs('#tg-tools').addEventListener('change', e => qs('#cc-stream').classList.toggle('hide-tools', !e.target.checked));
  qs('#tg-sys').addEventListener('change', e => qs('#cc-stream').classList.toggle('hide-sys', !e.target.checked));

  const filter = qs('#cc-filter');
  for (const n of NODES) {
  ...
  filter.addEventListener('change', () => {
    ...
    qsa('.cc-node').forEach(sec => sec.classList.toggle('filtered-out', f !== 'all' && sec.dataset.node !== f));
  });

  qs('#btn-replay').addEventListener('click', replay);

  qs('#cc-composer').addEventListener('submit', e => {
    e.preventDefault();
    const input = qs('#cc-input');
    const text = input.value.trim();
    if (!text) return;
    input.value = '';
    streamSys('You: “' + text + '” — queued; the run\'s agent sees it on the next resume.');
  });
...
function replay() {
  ...
  qs('#cc-stream').innerHTML = '';
  qs('#panel-header').innerHTML = '';
  qs('#panel-body').innerHTML = '';
  syncPanelLayout();

  const badge = qs('#run-status');
  badge.classList.remove('status-completed', 'status-failed', 'status-awaiting');
  badge.classList.add('status-running');
  qs('.sb-label', badge).textContent = 'Running';

  buildGraph();
  if (!qs('#cc-graph').classList.contains('hidden')) fitGraph();
  syncAwaiting();
  simulate();
}
