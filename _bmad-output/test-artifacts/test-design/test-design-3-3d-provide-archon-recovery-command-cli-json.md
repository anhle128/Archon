...
| 3.3D-UNIT-018     | AC2, R-004/R-012      | Unit                 | Whole-run retry of cancelled run returns `UNEXPECTED_STATE`, exit 78, and does not spawn. Cancelled runs require targeted retry with `--node`.                                                                                               | Dev               | R1-F17: cancelled whole-run retry was rejected (R1-F7). |
