...
assistants:
  ...
  codex:
    model: gpt-5.6-sol
    modelReasoningEffort: medium  # Legacy provider default; raw non-empty string
...
- `PATCH /api/auth/me/ai-prefs/tiers` / `…/aliases` - Per-key merge writes (`null` unsets); validate the provider via `isRegisteredProvider`, preserve any non-empty effort string exactly, and validate alias names (`@` prefix, not a reserved tier keyword). All return the updated prefs.
