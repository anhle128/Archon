...
- **`retry-node` can now rerun a completed node, not only a failed one.** Manual node retry was restricted to a `failed` node in a `failed`/`cancelled` run, so a node that errored but was recorded as `completed` — or any node whose already-successful output later turned out wrong — could not be rerun without discarding the whole run. Retry now also accepts a `completed` node and a `completed` run: it invalidates the target plus its current DAG descendants and reruns that branch with a fresh retry epoch exactly as before, and the Web UI shows the retry action on completed nodes too. Widening eligibility is the only change — `retry-node` stays the explicit, deliberate action with no new flag.
