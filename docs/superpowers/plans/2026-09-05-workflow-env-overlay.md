...
#### Release blockers as of post-Convergence-3 (historical; superseded by §E)

**None** *at Convergence 3 write time.*
...
**Historical §D Constitution Check owner sign-off (US-033 wording):** reviewers confirm (1) A, B, and D are present, (2) D’s then-current release-blocker list is empty or explicitly waived with Complexity Tracking, (3) US-020/US-021 rows cite inspect/hydrate and pending fail-closed evidence, (4) focused package tests above are green and the root validate legs recorded in the Focused/full validation row are green (`check:schema-upgrades` when Postgres is available remains the remaining CI-only schema leg). **Live release readiness is governed by §E below** — Convergence 4 later found post-CAS orphan and group-scope preflight gaps that D could not see.

---

### E. Post-Convergence-4 Re-Check (current)

_Recorded 2026-09-05 after US-034–US-037 landed (Ralph stories for Convergence 4 tasks 23–26) and their focused commands were re-run for this re-check (US-038 / task 27). Sections A–D above are **historical evidence** and are preserved verbatim except for authority pointers in C/D. This section is the single authoritative **current** release-blocker list._

**How to read this section:** D’s “Current release blockers: **None**” was true against the Convergence 3 tree. Convergence 4 tasks 23–24 reopened lifecycle/preflight constitution gaps that D could not see. Those gaps are **CLOSED** below with final symbols + focused tests. Any still-open item would appear only under “Current release blockers.” Lifecycle gate V cannot read PASS while the post-CAS orphan path (task 23 / US-034) remains open; container/preflight truthfulness cannot read PASS while group-scope preflight (task 24 / US-035) remains open.

#### Convergence 4 findings → current disposition

| Convergence 4 finding | Current status | Final code evidence | Focused test evidence (2026-09-05 re-run) |
| --- | --- | --- | --- |
| **US-034 / task 23 — Post-CAS continuation notices best-effort** | **CLOSED** | `packages/core/src/orchestrator/orchestrator-agent.ts`: after winning `hydrateResumableRun` CAS, supplied-input and ignored-ENV `platform.sendMessage` calls are `.catch` + `getLog().warn` (`orchestrator.resume_ignored_supplied_inputs_notice_failed` / `orchestrator.resume_ignored_supplied_env_notice_failed`) with run/env ids only; `executeWorkflow` still owns the already-`running` row. Notices stay *after* claim; losing-CAS path unchanged. | `packages/core/src/orchestrator/orchestrator-agent.test.ts` — `US-034: supplied-input notice rejection still dispatches after winning resume CAS`; `US-034: ignored-ENV notice rejection still dispatches stored overlay after winning resume CAS` (**255 pass**, 0 fail). |
| **US-035 / task 24 — Container preflight follows loop-group scope** | **CLOSED** | `packages/workflows/src/dag-executor.ts` `collectContainerIncompatibleProviders(nodes, scope, assistantModels, aiProfile)`: for each `loop_group`, derive body scope via `resolveGroupModelScope` and recurse; never `check()` the group container (it never calls `sendQuery`). `executeDagWorkflow` builds the outer preflight scope from the same provider/model/preset/tier fields runtime uses. | `packages/workflows/src/dag-executor.test.ts` — `collectContainerIncompatibleProviders` unit cases (outer codex + group claude; group→codex; nested inheritance; model-alias; approval/plannotator) + `executeDagWorkflow -- container preflight group scope` (**547 pass** with file). `node-model-resolution.test.ts` shared resolver (**23 pass**). |
| **US-036 / task 25 — Preset thinking warnings reach runtime** | **CLOSED** | `packages/workflows/src/dag-executor.ts` capability warning layer: thinking `isSet` is `request.thinkingUnsupported` (not node/workflow-only). Pure path already folds node/workflow/preset into `appliedThinking` + `thinkingUnsupported`; nodeConfig still receives `request.appliedThinking` so metadata/send stay truthful while `dag.unsupported_capabilities` fires once. | `node-model-resolution.test.ts` — `preset-only unsupported thinking sets thinkingUnsupported and keeps requested thinking`; supported Claude control. `dag-executor.test.ts` — Codex tier / OpenCode alias / Grok tier preset thinking warn once; Claude supported control (**23 + 547 pass**). |
| **US-037 / task 26 — Loop-group provider conflict warning preserved** | **CLOSED** | `packages/workflows/src/node-model-resolution.ts`: `ResolvedGroupModelScope` + `resolveGroupModelScope` forwards `resolveNodeModel().providerConflict`. `dag-executor.ts`: shared `warnModelProviderConflict`; loop_group branch emits via `groupScope.providerConflict` before body execution. No group container row in ENV `resolved` metadata. | `node-model-resolution.test.ts` — `exposes providerConflict when group provider disagrees with alias model` + nested/matching controls. `dag-executor.test.ts` — `warns once when loop_group provider conflicts with alias model (no-ENV)`; `warns once for ENV-overlaid loop_group provider/model conflict`. `executor.test.ts` — `US-037: ENV group provider/model conflict reaches snapshot body rows without group container` (**23 + 547 + 101 pass**). |
| **US-038 / task 27 — Constitution Check reconciliation** | **CLOSED** (this §E) | Plan §A–§D preserved; this dated §E is the sole current blocker list. | `git diff --check -- docs/superpowers/plans/2026-09-05-workflow-env-overlay.md`; manual compare to `.specify/memory/constitution.md` 154–156 and 172–175. |

#### Gate re-check (constitution lines 154–156 process + 172–175 compliance)

| Gate | Current verdict | Concrete repository evidence | Violation? | Accepted-use / rollback |
| --- | --- | --- | --- | --- |
| **II. Package-layer direction** | PASS | Unchanged: pure apply/resolution in `@archon/workflows` (`env-overlay.ts`, `node-model-resolution.ts`, `dag-executor.ts`, executor ownership); CRUD + run metadata in `@archon/core`; OpenAPI in `@archon/server`; console via `@/lib/api.generated` + local parsers only (no `@archon/workflows` import in web). Convergence 4 fixes stayed inside existing package owners. | None. | Revert ENV commits; additive table unused. |
| **III. Typed / OpenAPI schema rules** | PASS | No new OpenAPI surface in Convergence 4. Existing engine/core/server Zod + `registerOpenApiRoute` CRUD/preview; Start multipart exception unchanged. | None new. | Remove ENV routes/schemas. |
| **IV. Additive schema + generated files** | PASS | `remote_agent_workflow_envs` still additive-only; trailing indexes/comments; SQLite twin; bundled schema + `api.generated.d.ts`; inventory 21. Convergence 4 added no schema/migration. | None. | Leave table; stop writes. |
| **V. Fail-closed lifecycle** | PASS (Convergence 4 blockers closed) | Pre-isolation overlay gate; inspect-before-claim (US-020); inspect-before-stored-parse (US-029); pending+running `failWorkflowRun` + preamble fail-closed (US-021); complete snapshot before DAG; resume ignores new ENV (US-012); **post-CAS notices best-effort so executor always owns a claimed continuation (US-034)**; **container preflight matches runtime group scope (US-035)**; no autonomous abandon of ambiguous non-terminal rows. | None current. Task 23 orphan path and task 24 preflight lie are closed with evidence above — lifecycle gate must not read PASS while either remains open. | Disable ENV selection; abandon/finish overlay-bearing runs before binary rollback. |
| **VI. Secret-safe observability** | PASS | Errors/logs: env id/name/codes/run ids only (including US-034 notice-failure warns); list omits patches; run UI never renders prompt/bash; defensive raw keys; conflict/capability warnings carry declared/resolved provider ids and model refs — never prompt/bash bodies. | None. | Delete ENV rows; do not put secrets in prompt/bash. |
| **IV. Focused / full validation** | PASS (commands remain the gate) | Focused re-run 2026-09-05 for Convergence 4 owners: core `orchestrator-agent.test.ts` **255 pass**; workflows `node-model-resolution.test.ts` **23**, `dag-executor.test.ts` **547**, `executor.test.ts` **101** (all 0 fail). Prior §D package/root green legs remain the Phase 8 baseline. **Final shipping gate (not a substitute for this evidence):** root `bun run validate` **plus** the PostgreSQL `bun run check:schema-upgrades` CI leg when Postgres is available. `check:schema-upgrades` is CI/Postgres-live — not a local constitution code gap. Root `bun test` still forbidden. Ordinary PR process (template, `Closes` linkage, reviewer pass) is **not** a constitutional blocker. | None. | N/A |
| **I. Single-developer / no ACL creep** | PASS | No per-ENV ACL; provenance-only `created_by_user_id`; install-wide API gate only. | None. | N/A |
| **Intentional complexity** | WAIVED items still justified | Prior A/B/D Complexity Tracking rows remain. Convergence 4 deltas below are required safety restorations, not new product surface. | No new unjustified complexity. | Same rollback paths as A/B/D tables. |
| **UI/brand** | PASS | No Convergence 4 UI/token changes. | None. | Hide ENV UI. |
| **Workflow Language Constitution** | PASS (out of surface) | Still invocation/storage overlay; no YAML language ENV field. Group model forwarding + conflict warning restore baseline loudness — not new language surface. | None. | N/A |
| **Plan Quality Gate (const. 154–156)** | PASS | Pre-Phase-0 = §A; post-design = §B (historical); post-Convergence-3 = §D (historical); **post-Convergence-4 = this §E (current)**. | None. | N/A |

#### Complexity Tracking (post-Convergence-4) — deltas only

| Complexity | Status after Convergence 4 | Still needed? | Simplification if wrong |
| --- | --- | --- | --- |
| Best-effort post-CAS continuation notices (US-034) | Live `.catch` + warn after winning hydrate CAS; executor still dispatched | Yes — informational transport must not strand `running` without owner | Re-awaiting bare `sendMessage` after CAS reopens task 23 orphan |
| Group-scope container preflight walker (US-035) | Live `resolveGroupModelScope` recurse; skip group container check | Yes — preflight must match runtime body provider | Checking container provider / outer-only body walk reopens task 24 lie |
| Runtime consumes `thinkingUnsupported` (US-036) | Live capability table isSet | Yes — preset thinking must warn like node/workflow thinking | Node/workflow-only isSet re-hides preset warnings |
| Group `providerConflict` forward + shared warn (US-037) | Live on `ResolvedGroupModelScope` + `warnModelProviderConflict` | Yes — body inherits resolved provider and cannot recover conflict | Dropping conflict at group scope re-hides baseline safety signal |

#### Accepted risks / waivers (with rollback)

| Risk / waiver | Status | Rationale | Rollback / simplification |
| --- | --- | --- | --- |
| **Mixed-version resume for overlay-bearing runs** | Accepted **ops deployment constraint** (not an open constitution violation) | Old executors cannot replay complete ENV snapshots; Deployment § lines 751–761 / Known Risks. Code paths fail closed on the current binary. | Finish or abandon overlay-bearing non-terminal runs before rolling back the binary; disable ENV selection on mixed fleets. |
| **`bun run check:schema-upgrades` is CI/Postgres-live** | Accepted process split | Needs reachable PostgreSQL; not part of default local `bun run validate`. Schema remains additive-only with trailing index/comment placement. | Run the CI job (or local Postgres) before merge when schema touched; this feature’s table already landed additively. |
| **Prompt/bash plaintext in ENV patches** | Accepted product fact | Not a secret store; UI states install-visible plaintext. | Delete ENV rows; never put secrets in prompt/bash patches. |

#### Current release blockers (authoritative)

**None.**

All Convergence 4 constitutional gaps (task 23 post-CAS orphan / US-034; task 24 group-scope preflight / US-035) and the related warning-fidelity items (US-036 preset thinking; US-037 group providerConflict) are closed with the code and focused-test evidence above. Historical B lifecycle blockers (US-020/US-021) and Convergence 3 hardenings remain closed per §D. Remaining work for shipping is **ordinary PR process** (PR template, `Closes` linkage, human reviewer pass) plus the **final validation gate**: `bun run validate` and the PostgreSQL `check:schema-upgrades` CI leg — these are release process, **not** open constitution principle violations.

#### Verification of §E

- Manual comparison against `.specify/memory/constitution.md` lines 154–156 (documented pre-Phase-0 + post-design checks; this file now also carries post-Convergence-3 §D and post-Convergence-4 §E) and lines 172–175 (package boundaries, type/schema contracts, validation commands, git lifecycle safety, observability, secret handling).
- Historical A–D preserved; verdicts closed only where the table cites final symbols and re-run focused tests (not prior Ralph pass counts alone).
- `git diff --check -- docs/superpowers/plans/2026-09-05-workflow-env-overlay.md` must be clean.
- This section does **not** change implementation code (US-038 scope = plan evidence only).

**Current Constitution Check owner sign-off for PR:** reviewers confirm (1) A, B, D, and E are present, (2) **E’s** current release-blocker list is empty or explicitly waived with Complexity Tracking + Accepted risks, (3) US-034/US-035 rows cite best-effort post-CAS notices and group-scope preflight evidence (lifecycle/preflight gates cannot PASS while those were open), (4) US-036/US-037 warning-fidelity rows cite final symbols + focused tests, (5) focused Convergence 4 package tests above are green, and (6) shipping uses `bun run validate` plus the PostgreSQL `check:schema-upgrades` CI leg — ordinary PR process is distinct from constitutional blockers.
...
_Disposition (2026-09-05, US-038):_ Convergence 4 tasks 23–26 closed in code with focused re-runs recorded in Constitution Check **§E**. Task 27 is satisfied by appending §E; §E is the sole current release-blocker list (**None**). This Convergence 4 section remains historical gap text only.
