...
- `curl` `http://127.0.0.1:<port>/api/health` (default product port 3090; `verify-archon launch` default 13090; `prove` defaults to 13390).
...
- **Instance doctor.** Ask whether this process is worth driving. Run `verify-archon doctor --json`. Exit code `0`, `ok` is true, `baseUrl` matches the launched port (or the Mini URL), and `checks` includes `{ "name": "health", "status": "pass" }` (the HTTP body `status` is `"ok"`; it is not a top-level `health` object).
