...
describe('piModelInfoSchema catalog cost shape', () => {
  test('accepts cache rates and optional request-wide tiers', async () => {
    const { piModelInfoSchema } = await import('./schemas/provider.schemas');
    const parsed = piModelInfoSchema.parse({
    ...
    expect(parsed.cost.cacheRead).toBe(0.3);
    expect(parsed.cost.cacheWrite).toBe(3.75);
    expect(parsed.cost.tiers?.[0]?.inputTokensAbove).toBe(200_000);
  });

  test('rejects missing cache rates', async () => {
    const { piModelInfoSchema } = await import('./schemas/provider.schemas');
    const result = piModelInfoSchema.safeParse({
      ref: 'x/y',
      provider: 'x',
      id: 'y',
      name: 'y',
