...
describe('per-user AI prefs in chat + tier-fallback nudge', () => {
  ...
  test('uses assistant model as tier fallback for qodercli direct chat and title generation', async () => {
    ...
    expect(mockGenerateAndSetTitle).toHaveBeenCalledWith(
      ...
      undefined,
      expect.objectContaining({ model: 'qoder-pro', modelReasoningEffort: 'high' }),
      expect.objectContaining({
        model: 'qoder-pro',
        assistantConfig: expect.objectContaining({
          model: 'qoder-pro',
          modelReasoningEffort: 'high',
        }),
      })
...
describe('resolveTitleRequest', () => {
  ...
  test('resolves the built-in codex small tier instead of the raw config-default model', async () => {
    ...
    // Preset effort is carried on the portable field; Codex maps it at send time.
