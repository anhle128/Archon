...
- **PR-1 (merged): the data layer.** The five node variants the production
  builder could not originally represent — `loop`, `route_loop`, `approval`,
  `cancel`, `script` — plus the three existing kinds (`prompt`, `bash`,
  `command`), round-trippable with pure-function validation and typed fixtures.
