...
function routeEvent(overrides: {
  ...
  return {
    ...
    step_index: null,
    step_name: overrides.step_name ?? 'router',
    data: {
      sources: ['review'],
      outcome: 'negative',
      to: 'fix',
...
describe('selectNodeStdout', () => {
  ...
  test('does not leak stdout from a successful attempt or another node', () => {
    const events = [
      workflowEvent({
        id: 'other-done',
        step_name: 'other',
        event_type: 'node_completed',
        data: { type: 'bash', node_output: 'other node' },
      }),
      workflowEvent({ id: 'start-old' }),
      workflowEvent({
        id: 'done-old',
  ...
  test('validates bash truncation metadata and ignores malformed byte counts', () => {
    const completedRow: LogRow = { ...ROW, status: 'completed' };
    const events = [
      workflowEvent({ id: 'start-new' }),
      workflowEvent({
    ...
    for (const originalBytes of [-1, 1.5, '40000'] as const) {
      const malformed = [
        workflowEvent({ id: 'start-new' }),
        workflowEvent({
          id: 'done-new',
      ...
      expect(selectNodeStdout(malformed, completedRow).originalBytes).toBe(null);
      expect(selectNodeStdout(malformed, completedRow).truncated).toBe(true);
    }
  });
});
...
describe('selectGateChrome', () => {
  ...
  test('suppresses incompatible or resolved gate actions and shows inactive notice', () => {
    ...
    for (const variant of variants) {
      ...
      expect(chrome.canDecide).toBe(false);
      expect(chrome.showInactiveNotice).toBe(variant.notice);
    }
  });
  ...
  test('exposes a paused Plannotator review URL for the selected gate', () => {
    expect(
      selectGateChrome({
        definitionNode: {
          id: 'review',
          plannotator_gate: { document: 'plan.md', rework: { prompt: 'Fix' } },
        },
        events: [workflowEvent({ id: 'gate-start', step_name: 'review' })],
        row: GATE_ROW,
        approval: {
...
describe('selectRouteDecision', () => {
  test('selects the exact route execution among two decisions', () => {
    const events = [
      routeEvent({
        id: 'route-1',
        executionSeq: 1,
        data: { outcome: 'negative', to: 'fix', condition_result: false, attempt: 1 },
      }),
      routeEvent({
        id: 'route-2',
        executionSeq: 2,
  ...
  test('returns null for a missing route execution', () => {
    expect(
      selectRouteDecision([routeEvent({ id: 'route-1', executionSeq: 1 })], {
        ...ROUTE_ROW,
...
describe('selectLoopGroupChrome', () => {
  test('builds loop-group body and two iteration states in authored order', () => {
    const events = [
      workflowEvent({
        id: 'group-start',
