...
function routeEvent(overrides: {
  ...
  return {
    ...
    created_at: overrides.created_at ?? '2026-06-21T00:00:00.000Z',
  };
}

describe('buildLogRows', () => {
  test('ordinary rows use the exact server status and last node_started id', () => {
  ...
  test('two loop iterations remain separate rows whose ids are the originating start events', () => {
    const rows = buildLogRows(
  ...
  test('iteration completion and failure determine iteration status without changing origin ids', () => {
    const rows = buildLogRows(
  ...
  test('two route decisions remain separate completed rows whose ids are the originating events', () => {
    const rows = buildLogRows(
  ...
  test('suppresses the ordinary row for a loop node when iteration rows exist', () => {
    const rows = buildLogRows(
      [nodeState({ nodeId: 'loop', name: 'Loop', status: 'running' })],
      [
        workflowEvent({ id: 'node-start', event_type: 'node_started', step_name: 'loop' }),
        workflowEvent({
          id: 'loop-start-1',
          event_type: 'loop_iteration_started',
          step_name: 'loop',
    ...
    expect(rows.map(row => row.id)).toEqual(['loop-start-1']);
    expect(rows[0]?.selection).toEqual({ kind: 'loop_iteration', iteration: 1 });
  });

  test('suppresses the ordinary row for a route-loop node when route rows exist', () => {
    const rows = buildLogRows(
      [nodeState({ nodeId: 'router', name: 'Router', status: 'running' })],
      [
        workflowEvent({ id: 'node-start', event_type: 'node_started', step_name: 'router' }),
        routeEvent({ id: 'route-1', step_name: 'router', executionSeq: 1 }),
      ]
    );

    expect(rows.map(row => row.id)).toEqual(['route-1']);
    expect(rows[0]?.selection).toEqual({ kind: 'route_iteration', executionSeq: 1 });
  });

  test('rows interleave by source event-array order instead of grouping all base rows first', () => {
    const rows = buildLogRows(
      [
  ...
  test('fallback ids sit after all event-backed rows', () => {
    const rows = buildLogRows(
      [
        nodeState({ nodeId: 'pending', name: 'Pending', status: 'pending' }),
        nodeState({ nodeId: 'started', name: 'Started', status: 'running' }),
      ],
      [workflowEvent({ id: 'start-1', event_type: 'node_started', step_name: 'started' })]
    );
