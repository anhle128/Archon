...
import { evaluateCondition, InputRefError, serializeSafeCondition } from './condition-evaluator';
...
describe('evaluateCondition — $INPUTS refs', () => {
  ...
  it('branches on a supplied input value', () => {
    ...
    expect(evaluateCondition("$INPUTS.mode == 'fast'", noOutputs, undefined, inputs).result).toBe(
      true
    );
    expect(evaluateCondition("$INPUTS.mode == 'slow'", noOutputs, undefined, inputs).result).toBe(
      false
    );
    expect(evaluateCondition("$INPUTS.mode != 'slow'", noOutputs, undefined, inputs).parsed).toBe(
      true
    );
  });
  ...
  it('supports numeric comparison on an input value', () => {
    expect(
      evaluateCondition("$INPUTS.limit > '5'", noOutputs, undefined, { limit: '10' }).result
    ).toBe(true);
  });
  ...
  it('combines with node-output refs in a compound expression', () => {
    ...
    expect(
      evaluateCondition(
        "$INPUTS.mode == 'fast' && $gate.output.verdict == 'go'",
        outputs,
        undefined,
        { mode: 'fast' }
      ).result
  ...
  it('resolves an input literally named "output" instead of coincidentally returning empty', () => {
    ...
    expect(
      evaluateCondition("$INPUTS.output == 'v'", noOutputs, undefined, { output: 'v' }).result
    ).toBe(true);
  });

  it('THROWS on an undeclared input rather than resolving to empty', () => {
    expect(() =>
      evaluateCondition("$INPUTS.mode == ''", noOutputs, undefined, { other: 'x' })
    ).toThrow(InputRefError);
    expect(() =>
      evaluateCondition("$INPUTS.mode == 'fast'", noOutputs, undefined, { other: 'x' })
    ).toThrow(/Available inputs: \$INPUTS\.other\./);
  });
  ...
  it('offers a did-you-mean hint for a near-miss input name', () => {
    expect(() =>
      evaluateCondition("$INPUTS.mdoe == 'fast'", noOutputs, undefined, { mode: 'fast' })
    ).toThrow(/Did you mean \$INPUTS\.mode\?/);
  });
});
