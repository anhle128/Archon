...
// Known effort suggestions are shared, but providers remain authoritative for
// future values that are not in this installed Archon version.
...
describe('resolvePresetEffort', () => {
  ...
  test('passes a future provider-owned value through', () => {
    expect(resolvePresetEffort('claude', 'ultra')).toEqual({ ok: true });
  });
});
...
describe('isEffortValidForProvider', () => {
  ...
  test('accepts a future provider-owned value', () => {
    expect(isEffortValidForProvider('claude', 'ultra')).toBe(true);
  });
