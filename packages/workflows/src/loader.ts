...
export function validateDagStructure(
  ...
  // approval.message/on_reject.prompt, plannotator-gate fields, cancel, loop.prompt,
  // loop.until_bash, loop_group.until_bash, workflow.input/with/fan_out.items). A dangling ref in any of
  ...
  for (const node of nodes) {
    ...
    if (isPlannotatorGateNode(node)) {
      if (node.plannotator_gate.document !== undefined) {
        sources.push({ field: 'plannotator_gate.document', text: node.plannotator_gate.document });
      }
      if (node.plannotator_gate.prepare !== undefined) {
        sources.push({
          field: 'plannotator_gate.prepare.prompt',
          text: node.plannotator_gate.prepare.prompt,
        });
      }
      if (node.plannotator_gate.message !== undefined) {
        sources.push({ field: 'plannotator_gate.message', text: node.plannotator_gate.message });
      }
      sources.push({
        field: 'plannotator_gate.rework.prompt',
        text: node.plannotator_gate.rework.prompt,
      });
    }
    if (isLoopNode(node)) {
...
export function parseWorkflow(content: string, filename: string): ParseResult {
  try {
    ...
    let declaredEffort: string | undefined;
