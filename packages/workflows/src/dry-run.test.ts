...
describe('dryRunWorkflow', () => {
  test('fails explicitly for reachable route_loop nodes', async () => {
    const workflow = makeTestWorkflow({
      name: 'route-loop',
      nodes: makeRouteLoopWorkflowNodes(),
    });
    ...
    const result = await dryRunWorkflow({
      ...
      userMessage: '',
      cwd: process.cwd(),
      stubs: { fix: 'fixed', review: { approved: true } },
    });

    expect(result.outcome).toBe('failed');
    expect(result.trace.find(entry => entry.nodeId === 'review-router')).toMatchObject({
      state: 'failed',
      nodeType: 'route_loop',
      reason: 'Dry-run does not execute reachable route_loop nodes',
    });
  });
