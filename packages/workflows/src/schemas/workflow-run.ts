...
// ---------------------------------------------------------------------------
// ReviewFeedback — inline annotation submission and receipt
// ---------------------------------------------------------------------------

export const reviewFeedbackReceiptStatusSchema = z.enum([
  'accepted',
  'claimed',
  'processed',
  'superseded',
...
export type ReviewFeedbackReceiptStatus = z.infer<typeof reviewFeedbackReceiptStatusSchema>;

/** Trust-boundary limit for inline review feedback. Zod `.max()` is UTF-16. */
export const REVIEW_FEEDBACK_MAX_UTF8_BYTES = 16 * 1024;

export const reviewFeedbackTextSchema = z
  .string()
  .min(1)
  .superRefine((value, ctx) => {
    if (Buffer.byteLength(value, 'utf8') > REVIEW_FEEDBACK_MAX_UTF8_BYTES) {
      ctx.addIssue({
        code: 'custom',
        message: `Feedback exceeds ${String(REVIEW_FEEDBACK_MAX_UTF8_BYTES)} UTF-8 bytes`,
      });
    }
  });

export const reviewFeedbackSubmissionSchema = z.object({
  requestId: z.string().uuid(),
  reviewSessionId: z.string().uuid(),
  nodeId: z.string().min(1),
  gateId: z.string().min(1),
  feedback: reviewFeedbackTextSchema,
  status: reviewFeedbackReceiptStatusSchema,
  submittedAt: z.string().datetime(),
  claimedAt: z.string().datetime().optional(),
  processedAt: z.string().datetime().optional(),
  source: z.enum(['inline', 'native']).optional(),
});

export type ReviewFeedbackSubmission = z.infer<typeof reviewFeedbackSubmissionSchema>;
...
export interface ApprovalContext {
  ...
  /**
   * Plannotator gate only. A UUID generated by `transitionPlannotatorGate` each
   * time the gate enters `waiting_decision` phase. Published only while the phase
   * is `waiting_decision`; cleared (null) on every other phase transition so a
   * stale session ID can never be reused by a replacement annotate child.
   * Older contexts without a session ID remain readable but cannot accept inline
   * review feedback until a new annotate session is started.
   */
  reviewSessionId?: string | null;
  /**
   * Plannotator gate only. The most recently accepted inline feedback submission
   * awaiting supervisor consumption. Cleared on every non-waiting_decision phase
   * transition so stale submissions cannot bleed into a replacement session.
   * Null = no pending submission. Status tracks the lifecycle: accepted → claimed
   * → processed | failed | superseded.
   */
  feedbackSubmission?: ReviewFeedbackSubmission | null;
  /**
   * Plannotator gate only. Once either an inline or native decision is claimed,
   * this marker is set so the other source knows it lost the race. Cleared on
   * every non-waiting_decision transition together with reviewSessionId /
   * feedbackSubmission so a new review cycle starts clean.
   */
  decisionClaim?: { source: 'inline' | 'native'; requestId?: string; claimedAt: string } | null;
}
