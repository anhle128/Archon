  ...
  .superRefine((data, ctx) => {
    ...
    if (modeCount > 1) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message:
          "'command', 'prompt', 'bash', 'loop', 'route_loop', 'loop_group', 'approval', 'cancel', 'script', and 'include' are mutually exclusive",
      });
      return z.NEVER;
    }
    ...
    if (modeCount === 0) {
      ...
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message:
          "must have either 'command', 'prompt', 'bash', 'loop', 'route_loop', 'loop_group', 'approval', 'cancel', 'script', or 'include'",
      });
      return z.NEVER;
    }
