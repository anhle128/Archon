...
describe('LOOP_GROUP_NODE_AI_FIELDS', () => {
  ...
  test('differs from LOOP_NODE_AI_FIELDS on per-sendQuery fields', () => {
    // A plain loop calls sendQuery itself, so these fields are honored there.
    ...
    expect(
      LOOP_GROUP_NODE_AI_FIELDS.filter(f => f !== 'pi' && f !== 'effort' && f !== 'output_format')
    ).toEqual([...LOOP_NODE_AI_FIELDS]);
  });
});
