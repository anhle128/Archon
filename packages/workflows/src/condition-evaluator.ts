...
export function extractConditionReferences(expr: string): ConditionReferenceParseResult {
  ...
  for (const orClause of orClauses) {
    ...
    for (const atomText of andAtoms) {
      const atom = parseWhenAtom(atomText);
      if (!atom) {
        return { parsed: false, error: `condition atom could not be parsed: ${atomText}` };
      }
      if (atom.ref.kind === 'node') {
...
function serializeSafeAtom(atom: string): string {
  ...
  const parsed = parseWhenAtom(trimmed);
  if (!parsed) return redactConditionLiterals(trimmed);

  const ref = trimmed.slice(0, trimmed.indexOf(parsed.operator)).trim();
  const redacted = trimmed.endsWith("'") ? "'<redacted>'" : '<redacted>';
  return `${ref} ${parsed.operator} ${redacted}`;
}
...
function evaluateAtom(
  expr: string,
  nodeOutputs: Map<string, NodeOutput>,
  knownNodes: ReadonlyMap<string, readonly string[] | undefined> | undefined,
  inputs: Record<string, string> | undefined
  ...
  if (ref.kind === 'node') {
    const producer = nodeOutputs.get(ref.nodeId);
    const declaredFields = knownNodes?.get(ref.nodeId);
    if (
      ref.field !== undefined &&
      declaredFields !== undefined &&
      !declaredFields.includes(ref.field)
    ) {
      throw new OutputRefError(ref.nodeId, ref.field, 'not-in-schema');
    }
    if (
      knownNodes?.has(ref.nodeId) &&
      (producer === undefined ||
      ...
      getLog().debug(
        { nodeId: ref.nodeId, field: ref.field ?? null },
        'condition_producer_unavailable'
      );
...
export function evaluateCondition(
  ...
  for (const orClause of orClauses) {
    ...
    for (const atom of andAtoms) {
      const { result, parsed } = evaluateAtom(atom, nodeOutputs, knownNodes, inputs);
