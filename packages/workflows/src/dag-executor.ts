...
async function createPreNodeCheckpoint(params: {
  ...
  // Folder projects and other non-git cwd values have no checkout to snapshot.
  // Retry refs are git objects (`refs/archon/checkpoints/...`); requiring a
  // repo here would fail `--folder` / `--container` runs before the first node.
  if (!(await isGitWorkTree(toWorktreePath(params.cwd)))) {
    getLog().debug(
      { workflowRunId: params.workflowRun.id, nodeId: checkpointNodeId },
      'dag.checkpoint_skipped_not_git'
    );
    return;
  }
...
export function checkTriggerRule(
  ...
  const nodeDeps = activeDependencyIds ?? node.depends_on ?? [];
...
function collectPathNodesToAnyTarget(
  startId: string,
  targetIds: ReadonlySet<string>,
  dependents: Map<string, string[]>
): Set<string> | null {
  const reachablePaths = Array.from(targetIds, targetId =>
    collectPathNodesToTarget(startId, targetId, dependents)
  ).filter((path): path is Set<string> => path !== null);

  if (reachablePaths.length === 0) return null;
...
interface RouteRerunPlan {
  ...
  activeDependenciesByNode: Map<string, string[]>;
...
function buildSelectedRouteRerunPlan(params: {
  ...
  const activeDependenciesByNode = new Map<string, string[]>();
  const externalPrerequisites: RouteRerunExternalPrerequisite[] = [];
  if (!isRouteLoopNode(params.routeLoopNode)) {
    return { rerunNodeIds, activeDependenciesByNode, externalPrerequisites };
  }
  if (params.selectedTargetId !== params.routeLoopNode.route_loop.routes.negative) {
    return { rerunNodeIds, activeDependenciesByNode, externalPrerequisites };
  }

  const sourceNodeIds = new Set(params.routeLoopNode.depends_on ?? []);
  const rerunPath = collectPathNodesToAnyTarget(
    params.selectedTargetId,
    sourceNodeIds,
    params.dependents
  );
  if (rerunPath === null) return { rerunNodeIds, activeDependenciesByNode, externalPrerequisites };
  ...
  for (const nodeId of rerunPath) {
    ...
    if (activeDeps.length > 0) {
      activeDependenciesByNode.set(nodeId, activeDeps);
    }
  }
  rerunNodeIds.add(params.routeLoopNode.id);
  const activeRouteLoopSources = (params.routeLoopNode.depends_on ?? []).filter(dep =>
    rerunPath.has(dep)
  );
  if (activeRouteLoopSources.length > 0) {
    activeDependenciesByNode.set(params.routeLoopNode.id, activeRouteLoopSources);
  }
  return { rerunNodeIds, activeDependenciesByNode, externalPrerequisites };
}
      ...
      .createWorkflowEvent({
        ...
        data: withRetryEpochData(workflowRun, undefined, {
          ...
          ...(nodeResolvedModel
            ? {
                model_usage: {
                  requested: nodeObservability?.model,
...
async function executeBashNode(
  ...
  const { prompt: substitutedScript } = substituteWorkflowVariables(
    ...
    undefined,
    undefined,
    { shellSafe: true, stateDir, prRemote }
  );
...
interface RunLayersRouteState {
  initialBlockingRouteTargetIds: Set<string>;
  activatedRouteTargetIds: Set<string>;
  scheduledRouteRerunNodeIds: Set<string>;
  scheduledRouteRerunActiveDependencyIds: Map<string, Set<string>>;
  routeRerunScheduleVersions: Map<string, number>;
  nodeLayerIndex: Map<string, number>;
  nodesById: Map<string, DagNode>;
  dependents: Map<string, string[]>;
  routeLoopMetadata: RouteLoopRuntimeMetadata;
  scheduleRouteRerunNode(nodeId: string, activeDependencyIds?: readonly string[]): void;
  clearRouteRerunNode(nodeId: string): void;
  serializeRouteDecision<T>(operation: () => Promise<T>): Promise<T>;
}
...
async function runLayers(ctx: RunLayersContext): Promise<'completed' | 'pending'> {
  ...
  for (let layerIdx = 0; layerIdx < layers.length; layerIdx++) {
    ...
    const scheduledAtLayerStart = route
      ? new Set(route.scheduledRouteRerunNodeIds)
      : new Set<string>();
    const activeDependencyIdsAtLayerStart = route
      ? new Map(
          Array.from(route.scheduledRouteRerunActiveDependencyIds, ([nodeId, deps]) => [
            nodeId,
            new Set(deps),
          ])
        )
      : new Map<string, Set<string>>();
    const scheduleVersionsAtLayerStart = route
      ? new Map(route.routeRerunScheduleVersions)
      : new Map<string, number>();
    const activatedRouteTargetsAtLayerStart = route
      ? new Set(route.activatedRouteTargetIds)
      : new Set<string>();
    ...
    if (mutatesCheckout && layerExecutableNodes.length > 1) {
      getLog().warn(
        {
          workflowRunId: workflowRun.id,
          layerIdx,
    ...
    const isBlockedByInactiveRouteTarget = (
      ...
      if (
        route.initialBlockingRouteTargetIds.has(nodeId) &&
      ...
      if (visiting.has(nodeId)) return false;
      visiting.add(nodeId);
      const node = route.nodesById.get(nodeId);
      if (!node) {
        visiting.delete(nodeId);
        return false;
      }
    ...
    const layerResults = await Promise.allSettled(
      layer.map(async (node): Promise<LayerNodeResult> => {
        try {
          ...
          if (route && isBlockedByInactiveRouteTarget(node.id)) {
            return { nodeId: node.id, output: { state: 'pending' as const, output: '' } };
          }

          if (route && ctx.nodeOutputs.has(node.id) && !scheduledAtLayerStart.has(node.id)) {
            return {
              nodeId: node.id,
              output: ctx.nodeOutputs.get(node.id) ?? { state: 'pending' as const, output: '' },
            };
          }
          ...
          const activeDependencyIds = scheduledAtLayerStart.has(node.id)
            ? activeDependencyIdsAtLayerStart.get(node.id)
            : undefined;
          const triggerDecision = checkTriggerRule(
            node,
            ctx.nodeOutputs,
            activeDependencyIds ? Array.from(activeDependencyIds) : undefined
          );
          ...
          if (mutatesCheckout && isCheckpointableExecutableNode(node)) {
          ...
          if (isRouteLoopNode(node)) {
            if (!route) {
              return {
                ...
                output: {
                  ...
                  output: '',
                  error: `route_loop '${node.id}' reached a sub-DAG without route runtime state`,
                },
              };
            }

            const sourceNodeIds = node.depends_on ?? [];
            const requiredSourceNodeIds =
              activeDependencyIds !== undefined ? Array.from(activeDependencyIds) : sourceNodeIds;
            for (const sourceNodeId of requiredSourceNodeIds) {
              const sourceOutput = ctx.nodeOutputs.get(sourceNodeId);
              if (sourceOutput?.state !== 'completed') {
                return {
                  nodeId: node.id,
                  output: {
                    state: 'failed' as const,
                    output: '',
                    error: `route_loop '${node.id}' cannot evaluate condition because source dependency '${sourceNodeId}' has not completed successfully`,
                  },
                };
              }
            }
            ...
            const { transition, selectedRerunPlan } = await route.serializeRouteDecision(
              async () => {
                const nextTransition = applyRouteLoopTransition({
                  metadata: route.routeLoopMetadata,
                  routeLoopNodeId: node.id,
                  routeLoop: node.route_loop,
                  sourceNodeIds,
                  conditionResult,
                });
                assertRouteTargetCanBeActivated(nextTransition.eventData.to, ctx.nodeOutputs);
                const nextSelectedRerunPlan = buildSelectedRouteRerunPlan({
                  routeLoopNode: node,
                  selectedTargetId: nextTransition.eventData.to,
                  nodesById: route.nodesById,
                  dependents: route.dependents,
                });
                ...
                const expectedExecutionSeq = route.routeLoopMetadata.executionSeq;
                ...
                route.routeLoopMetadata = routeLoopRuntimeMetadataSchema.parse(
            ...
            route.activatedRouteTargetIds.add(transition.eventData.to);
            for (const rerunNodeId of selectedRerunPlan.rerunNodeIds) {
              route.scheduleRouteRerunNode(
                rerunNodeId,
    ...
    for (const result of layerResults) {
      if (result.status === 'fulfilled') {
        ...
        if (
          route &&
          scheduledAtLayerStart.has(nodeId) &&
          route.scheduledRouteRerunNodeIds.has(nodeId) &&
          route.routeRerunScheduleVersions.get(nodeId) ===
            scheduleVersionsAtLayerStart.get(nodeId) &&
          output.state !== 'pending'
        ) {
          route.clearRouteRerunNode(nodeId);
        }
      } else {
...
function isWorkflowRetryContextValue(value: unknown): value is WorkflowRetryContext {
  return (
    typeof value === 'object' &&
    value !== null &&
    'retryEpoch' in value &&
    'targetNodeId' in value &&
    'invalidatedNodeIds' in value
  );
}

function isExecutionContextValue(value: unknown): value is ExecutionContext {
  return typeof value === 'object' && value !== null && 'kind' in value;
}

function isContainerRunContextValue(value: unknown): value is ContainerRunContext {
  return typeof value === 'object' && value !== null && 'backend' in value && 'envId' in value;
}
...
export async function executeDagWorkflow(
  ...
  aiProfile?: ResolvedAiProfile,
  workflowPreset?: ModelAliasPreset,
  retryOrScopeArtifactsDir?: WorkflowRetryContext | string,
  /**
   ...
   */
  scopeOrExecContext?: string | ExecutionContext,
  /**
   ...
   */
  execOrContainerCtx?: ExecutionContext | ContainerRunContext,
  /**
   ...
   */
  maybeContainerCtx?: ContainerRunContext,
  /**
   * Injected closure that starts a child sub-run for a `workflow:` node (#2121
  ...
  const retryContext = isWorkflowRetryContextValue(retryOrScopeArtifactsDir)
    ? retryOrScopeArtifactsDir
    : undefined;
  const scopeArtifactsDir =
    typeof retryOrScopeArtifactsDir === 'string'
      ? retryOrScopeArtifactsDir
      : typeof scopeOrExecContext === 'string'
        ? scopeOrExecContext
        : undefined;
  const execContext = isExecutionContextValue(scopeOrExecContext)
    ? scopeOrExecContext
    : isExecutionContextValue(execOrContainerCtx)
      ? execOrContainerCtx
      : ({ kind: 'host' } as const);
  const containerCtx =
    maybeContainerCtx ??
  ...
  const scheduledRouteRerunActiveDependencyIds = new Map<string, Set<string>>();
  ...
  const scheduleRouteRerunNode = (
    nodeId: string,
    activeDependencyIds?: readonly string[]
  ): void => {
    scheduledRouteRerunNodeIds.add(nodeId);
    if (activeDependencyIds !== undefined && activeDependencyIds.length > 0) {
      scheduledRouteRerunActiveDependencyIds.set(nodeId, new Set(activeDependencyIds));
    } else {
      scheduledRouteRerunActiveDependencyIds.delete(nodeId);
    }
  ...
  const routeState: RunLayersRouteState | undefined = hasRouteLoopNodes
    ? {
        initialBlockingRouteTargetIds,
        activatedRouteTargetIds,
        scheduledRouteRerunNodeIds,
        scheduledRouteRerunActiveDependencyIds,
        routeRerunScheduleVersions,
        nodeLayerIndex,
        nodesById,
        dependents,
        routeLoopMetadata,
        scheduleRouteRerunNode,
        clearRouteRerunNode,
        serializeRouteDecision,
      }
    : undefined;
  ...
  const runCtx: RunLayersContext = {
    ...
    workflowLevelOptions,
    aiProfile,
    workflowPreset: effectiveWorkflowPreset,
    workflowTier,
    artifactsDir,
    stateDir,
    logDir,
    ...
    configuredCommandFolder,
    issueContext,
    mutatesCheckout: workflow.mutates_checkout !== false,
    retryContext,
    retryEpoch,
    persistScopeKey,
    workflowPersistSessions,
