...
Operator-authored **point-in-time cost estimates** live only in the home global
config (`~/.archon/config.yaml`, or `$ARCHON_HOME/config.yaml`). They never
belong in a repository `.archon/config.yaml`, never appear on `MergedConfig` /
`SafeConfig`, and are not writable through configuration mutation endpoints or
the web settings UI.

```yaml
# ~/.archon/config.yaml only — never in a repo config
pricing:
  models:
    - provider: openai
      model: gpt-5.4
...
- Each entry is an **exact** `(provider, model)` pair. Provider and model stay
  separate fields so either may contain `/` (for example OpenRouter-style ids).
- `provider` and `model` are trimmed non-empty strings. Duplicate pairs are
  invalid for estimation.
- Rate fields (`input`, `output`, `cacheRead`, `cacheWrite`) are optional finite
  non-negative numbers (USD per one million tokens). At least one rate is
  required per entry.
- Matching is exact only — no bare-model, prefix, substring, case-folded, or
  fuzzy match, and never a concatenated `provider/model` lookup key.

### When Archon estimates

Estimation runs only when a workflow AI pass records usage into the ledger:

1. If the provider observation already carries `costUsd` (including **zero**),
   that value is stored as reported USD and **no estimate** is computed.
2. Otherwise Archon looks up the exact global-config `(provider, model)` pair.
3. Otherwise it looks up the exact Pi catalog `provider` + `id` pair.
4. Otherwise the observation stays unpriced (`cost_estimated_usd` null).

Further limits:

- Both input and output token counts are required before any estimate.
- Cache rates apply only to cache dimensions the provider actually reported.
- If any positive reported category lacks a matching rate, the **whole**
  estimate is null (no known-partial number).
- Reasoning tokens are already inside output and are never charged again.
- Request-only observations stay unpriced in v1.
- Invalid or duplicate config pairs leave the observation unpriced and never
  fall through to the catalog for that pair.
- Estimates are operational, not invoice accounting. They are stored only on
  ledger rows (`cost_estimated_usd` + `pricing_source`), never in provider
  results, workflow-event JSON, legacy run totals, or reported `cost_usd`.
- Estimates never feed `maxBudgetUsd` or any budget-enforcement path.

### Restart semantics (no file watcher)

Global config and the merged Pi model catalog are **process-cached**. Editing
`pricing:` in `~/.archon/config.yaml` or changing `~/.pi/agent/models.json`
takes effect:

- on the next CLI invocation (fresh process), or
- after restarting a long-running `archon serve` / server process.

This feature does **not** add a file watcher. There is no automatic reprice of
historical ledger rows when rates or the catalog change.

See also: [`archon usage`](/reference/cli/#usage), console **Cost** page, and
[AI Assistants → Workflow usage and cost tracking](/getting-started/ai-assistants/#workflow-usage-and-cost-tracking).
