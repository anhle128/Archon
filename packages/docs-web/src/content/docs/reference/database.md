...
The database has **20 application tables** (all prefixed with `remote_agent_`) plus **4 PostgreSQL-only Better Auth tables**:
...
6. **`remote_agent_workflow_events`** - Step-level workflow event log
   ...
   - Internal `node_usage_recorded` events carry snake_case usage payloads for workflow AI passes (not mapped to the external event outbox or the high-frequency dashboard source poller)
...
7. **`remote_agent_usage_ledger`** - Normalized workflow usage observations
   - Child of `remote_agent_workflow_events` (`ON DELETE CASCADE`); one row per validated provider observation
   - Exactly 17 columns: event ownership + entry index, agent/upstream provider, model attribution, kind, token/cache/reasoning/request measures, reported vs estimated USD, and `pricing_source`
   - No duplicated run/node/workflow/codebase/user/timestamp identity columns — join through the parent event → run
   - CHECKs enforce measure presence, cost mutual exclusion, pricing_source↔estimate coupling, model nullability, and reasoning ≤ output
   - `UNIQUE(workflow_event_id, entry_index)`; query indexes on agent provider, upstream provider/model, usage-event `created_at`, and `workflow_runs(codebase_id)`
   - Point-in-time estimates materialize here only; there is no historical backfill or automatic re-ledger job

8. **`remote_agent_workflow_node_checkpoints`** - Per-node git checkpoints for manual DAG node retry
...
9. **`remote_agent_messages`** - Conversation message history
...
10. **`remote_agent_codebase_env_vars`** - Per-project env vars for workflow execution
...
11. **`remote_agent_users`** - Archon-internal user identity
...
12. **`remote_agent_user_identities`** - Platform-to-Archon user mapping
...
13. **`remote_agent_workflow_node_sessions`** - Per-node provider session IDs persisted across workflow re-runs
    - Opt-in via `persist_session`; keyed by `(workflow_name, node_id, scope_key, provider)`
    - `scope_key` is typically the conversation UUID

14. **`remote_agent_user_github_tokens`** - Per-user GitHub device-flow tokens
...
15. **`remote_agent_user_provider_keys`** - Per-user AI-provider credentials (API key or OAuth subscription blob)
...
16. **`remote_agent_user_ai_prefs`** - Per-user AI preferences (personal model tiers, `@custom` aliases, default assistant)
...
17. **`remote_agent_schema_version`** - Diagnostic schema vintage (not a migration ledger)
    - Single row recording which Archon build created the database and which last applied schema
    - Surfaced by `archon doctor` and `GET /api/health`; nothing gates on it

18–20. **`remote_agent_workflow_provider_bindings`**, **`remote_agent_workflow_event_outbox`**, **`remote_agent_workflow_event_delivery_attempts`** - Outbound external workflow-event binding, durable outbox, and delivery attempt history

21–24. **`remote_agent_auth_user` / `remote_agent_auth_session` / `remote_agent_auth_account` / `remote_agent_auth_verification`** - Better Auth tables for opt-in web login
...
> The `remote_agent_workflow_node_checkpoints` table, the `remote_agent_codebases.kind` column (project `'repo'` | `'folder'` discriminator, commented "From migration 024"), the `remote_agent_users.role` column, the `remote_agent_usage_ledger` table, and the four `remote_agent_auth_*` Better Auth tables (opt-in web login) are applied inline in `000_combined.sql` rather than as numbered migrations, and converge on startup via the idempotent schema apply.
