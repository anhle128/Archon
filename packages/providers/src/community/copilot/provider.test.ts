...
describe('CopilotProvider.sendQuery', () => {
  ...
  test('usage before sendAndWait reject yields terminal isError with every entry once', async () => {
    const session = makeFakeSession('sess-usage-late-reject');
    nextCreateSessionResult = session;
    ...
    // Missing token measures stay missing; requests still count.
    session.fire(evt('assistant.usage', { model: '  ', cacheWriteTokens: 7 }));
    session.rejectSend(new Error('upstream disconnect'));

    const firstResult = await first;
    const rest = await collect(gen);
    const chunks = [firstResult.value, ...rest].filter(
    ...
    expect(result).toMatchObject({
      type: 'result',
      isError: true,
      sessionId: 'sess-usage-late-reject',
      tokens: { input: 15, output: 5 },
    });
    expect(result.errors).toEqual(['upstream disconnect']);
    ...
    // Exactly one terminal result — no duplicate usage rows or second result.
    expect(chunks.filter(c => c.type === 'result')).toHaveLength(1);
    expect(session.disconnected).toBe(true);
  });

  test('usage-bearing late reject keeps friendly model-access classification', async () => {
    const session = makeFakeSession('sess-usage-model-access');
    nextCreateSessionResult = session;
    ...
    session.fire(evt('assistant.usage', { model: 'gpt-5', inputTokens: 2, outputTokens: 1 }));
    session.rejectSend(new Error('Model not available for this account'));
    ...
    expect(result).toMatchObject({
      type: 'result',
      isError: true,
      sessionId: 'sess-usage-model-access',
      tokens: { input: 2, output: 1 },
    });
    expect(result?.errors?.[0]).toMatch(/Copilot model access error/i);
    expect(result?.errors?.[0]).toMatch(/Try a different model/i);
    expect(result?.usageBreakdown).toEqual([
      {
        provider: 'github-copilot',
        model: 'gpt-5',
  ...
  test('session.error then successful fallback does not become false terminal failure', async () => {
    const session = makeFakeSession('sess-fallback-ok');
    nextCreateSessionResult = session;
    ...
    session.fire(evt('session.error', { errorType: 'transient', message: 'transient blip' }));
    session.fire(evt('assistant.usage', { model: 'gpt-5', inputTokens: 1, outputTokens: 1 }));
    session.fire(evt('assistant.message_delta', { messageId: 'm', deltaContent: 'recovered' }));
    session.resolveSend(undefined);
    ...
    expect(chunks).toContainEqual(
      expect.objectContaining({ type: 'assistant', content: 'recovered' })
    );
    const results = chunks.filter(c => c.type === 'result');
    expect(results).toHaveLength(1);
    expect(results[0]).toMatchObject({
      type: 'result',
      sessionId: 'sess-fallback-ok',
      tokens: { input: 1, output: 1 },
    });
    expect(results[0]).not.toHaveProperty('isError');
    expect(results[0]?.usageBreakdown).toHaveLength(1);
    expect(chunks).not.toContainEqual(
      expect.objectContaining({
        type: 'system',
        content: expect.stringContaining('transient blip'),
      })
    );
  });
});
